...
export async function getSKUs(req: Request, res: Response, next: NextFunction) {
  try {
    const skus = await prisma.sKU.findMany({ include: skuInclude, orderBy: { name: 'asc' } });
    res.json(skus);
  } catch (err) { next(err); }
}
...
export async function getSKU(req: Request, res: Response, next: NextFunction) {
  try {
    const sku = await prisma.sKU.findUniqueOrThrow({ where: { id: Number(req.params.id) }, include: skuInclude });
    res.json(sku);
  } catch (err) { next(err); }
}
...
export async function createSKU(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const sku = await prisma.sKU.create({ data, include: skuInclude });
...
export async function updateSKU(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const sku = await prisma.sKU.update({ where: { id: Number(req.params.id) }, data, include: skuInclude });
    res.json(sku);
  } catch (err) { next(err); }
}
...
export async function deleteSKU(req: Request, res: Response, next: NextFunction) {
  try {
    await prisma.sKU.delete({ where: { id: Number(req.params.id) } });
    res.status(204).send();
  } catch (err) { next(err); }
}
